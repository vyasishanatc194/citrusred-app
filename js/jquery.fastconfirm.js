	...
	$.fn.fastConfirm = function(options) {
		...
		return this.each(function() {
			var offset,
				...
				leftOffset,
				trigger = this,
				$confirmYes = $('<button class="' + params.fastConfirmClass + '_proceed btn confirm inline-block">' + params.proceedText + '</button>'),
				$confirmNo = $('<button class="' + params.fastConfirmClass + '_cancel btn cancel inline-block">' + params.cancelText + '</button>'),
				$confirmBox = $('<div class="' + params.fastConfirmClass + '"><div class="' + params.fastConfirmClass + '_arrow_border"></div><div class="' + params.fastConfirmClass + '_arrow"></div><h5>' + params.titleText + '</h5><p>' + params.questionText + '</p></div>'),
			...
			if (!$(trigger).data('fastconfirm_binded')) {
				...
				$buttonContainer.append($confirmYes).append($confirmNo);
				$confirmBox.append($buttonContainer);
