...
var nicEditorButton = bkClass.extend({
	...
	construct : function(e,buttonName,options,nicEditor) {
		...
		this.contain = new bkElement('DIV').setStyle({width : '32px', height : '30px'}).addClass('buttonContain').appendTo(this.margin);
		this.border = new bkElement('DIV').setStyle({backgroundColor : '#efefef', border : '1px solid #efefef'}).appendTo(this.contain);
		this.button = new bkElement('DIV').setStyle({width : '32px', height : '30px', overflow : 'hidden', zoom : 1, cursor : 'pointer'}).addClass('button').setStyle(this.ne.getIcon(buttonName,options)).appendTo(this.border);
...
var nicEditorPane = bkClass.extend({
	construct : function(elm,nicEditor,options,openButton) {
		...
		this.contain = new bkElement('div').setStyle({zIndex : '99999', overflow : 'hidden', 'position' : 'absolute', 'left' : posleft, 'top' : this.pos[1]+'px'})
	...
	mouseClick : function() {
		if(!this.isDisabled) {
			if(this.pane && this.pane.pane) {
				...
				this.pane = new nicEditorPane(this.contain,this.ne,{width : (this.width || '270px'), backgroundColor : '#fff',position:'fixed',left:'700px',top:'40px'},this);
				this.addPane();
...
var nicButtonTips = bkClass.extend({
	...
	create : function(button) {
		this.timer = null;
		if(!this.pane) {
			this.pane = new nicEditorPane(button.button,this.ne,{fontSize : '12px', marginTop : '5px',position:'fixed',top:'40px'});
...
var nicEditorSelect = bkClass.extend({
	...
	construct : function(e,buttonName,options,nicEditor) {
		...
		this.control = new bkElement('div').setStyle({overflow : 'hidden', 'float' : 'right', height: '30px', width : '32px'}).addClass('selectControl').setStyle(this.ne.getIcon('arrow',options)).appendTo(this.items);
	...
	//nicEditors.registerPlugin(nicPlugin,nicSelectOptions);
		this.pane = new nicEditorPane(this.items,this.ne,{width : '112px', padding: '0px', borderTop : 0, borderLeft : '1px solid #ccc', borderRight : '1px solid #ccc', borderBottom : '0px', backgroundColor : '#fff',position:'fixed',top:'40px'});

		for(var i=0;i<this.selOptions.length;i++) {
			var opt = this.selOptions[i];
			var itmContain = new bkElement('div').setStyle({overflow : 'hidden', borderBottom : '1px solid #ccc', width: '112px', textAlign : 'left', cursor : 'pointer'});
...
this.ne.selectedInstance.restoreRng();
			...
			var colorSelectedBox = new bkElement('INPUT').setAttributes({type:'text','id':'selectedcolor'}).setStyle({backgroundColor : '#ebebeb',color:'#000000',width : '80px', height : '20px','margin-top':'5px'}).appendTo(colorItems);
			var colorSelectedSubmit = new bkElement('INPUT').setAttributes({type:'button','value':'ok'}).setStyle({backgroundColor : '#ff6500',color:'#ffffff',width : '40px', height : '30px','margin-top':'5px','margin-left':'5px'}).addEvent('click',this.colorImplement.closure(this)).appendTo(colorItems);
